...
from utils.constant import (BACKGROUND_COLOR, BASELINE_COLOR, CAPTION_COLOR, CONTEXT_BACKGROUND_COLOR, DRAWING_COLOR,
                            ...
                            TITLE_COLOR, TEXT_BORDER_COLOR, DIAGRAM_COLOR, TABLE_COLOR, LINE_COLOR, COLUMN_COLOR)
from utils.constant import (BACKGROUND_LABEL, BASELINE_LABEL, CAPTION_LABEL, CONTEXT_BACKGROUND_LABEL, DRAWING_LABEL,
                            ...
                            TITLE_LABEL, TEXT_BORDER_LABEL, DIAGRAM_LABEL, TABLE_LABEL, LINE_LABEL, COLUMN_LABEL)
...
TABLE_LAYOUT_RANGE = {
    'col_size_range_old': (50, 400),
    'col_size_range_new': (30, 40),
...
class AbstractTextElement(AbstractElement):
    ...
    def format_text(self, text):
        if self.font_type in ['normal', 'handwritten']:
            ...
            text = google(text, src='en', dst='ar', proxies = {'http': '127.0.0.1:1080'})
            text = get_display(arabic_reshaper.reshape(text))
        elif self.font_type == 'chinese':
            text = google(text, src='en', dst='zh-CN', proxies = {'http': '127.0.0.1:1080'})
        else:
    ...
    def to_label_as_array(self):
        label = np.full(self.size, self.background_label, dtype=np.float64)
...
#Class corresponding to modern handwritten tables, with loose layouts
class TableElement_loose(AbstractElement):
    label = TABLE_LABEL
    color = TABLE_COLOR
    border_label = TEXT_BORDER_LABEL
    border_color = TEXT_BORDER_COLOR
    ...
    text_label = TABLE_WORD_LABEL
    text_color = TABLE_WORD_COLOR
    name = 'table'
    ...
    def generate_content(self):
        ...
        self.table_multi_labels = self.parameters.get('table_multi_labels', False)
        self.content = self.parameters.get('content', None)
        ...
        #Choose the opacity of the separators, and their presence or absence
        self.colors_alpha = self.colors + (self.opacity,)
        self.lines = choice([True, False], p = [0.7, 0.3])
    ...
    def _generate_table(self, dictionary):
        ...
        if(self.width > 1500):
            width, height = randint(min(1700, self.width), self.width), randint(min(1000, self.height), self.height)
        else:
            width, height = randint(min(700, self.width), self.width), randint(min(1000, self.height), self.height)
        
        line_size_min = round(self.font_size * 1.3)
        line_size_max = round(self.font_size * 2.5)
        lines = np.cumsum(np.random.randint(line_size_min, line_size_max, 60))
        lines = lines[lines < height - line_size_min].tolist()
            
        columns = np.cumsum(np.random.randint(*TABLE_LAYOUT_RANGE['col_size_range_old'], 30))
        columns = columns[columns < width - TABLE_LAYOUT_RANGE['col_size_range_old'][0]].tolist()

        words, word_positions = [], []
        for i, c in enumerate([0] + columns):
            presence_column = choice([False, True], p = [0, 1])
            if(presence_column):
                for j, l in enumerate([0] + lines):
                    presence_word = choice([True, False], p = [0.8, 0.2])

                    if(presence_word):
                        word_as_number = choice([True, False], p = [0.7, 0.3])
                        if((j == 0 and i == 0) or (j == 0 and i ==2)):
                            word_as_number = False
                        if word_as_number:
                            n_letter = randint(2, 9)
                            word = f'{randint(0, 10**n_letter - 1):,}'
                        else:
                            #Three words concatenation
                            word = ' '.join([rand_choice(dictionary), rand_choice(dictionary), rand_choice(dictionary)])
    ...
    def to_image(self):
        ...
        for l in self.table['lines']:
            ...
            opacity = randint(*NEG_ELEMENT_OPACITY_RANGE[self.name] if self.as_negative
                               else POS_ELEMENT_OPACITY_RANGE['line'])
                               
            
            self.line_params = {
                'width': 0,
                'fill': tuple([randint(0, 1)] * 3) + (opacity,),
            }
            if(self.lines):
    ...
    def to_label_as_array(self):
        label = np.full(self.size, BACKGROUND_LABEL, dtype=np.float64)
        ...
        if self.as_negative:
            ...
            if self.table_multi_labels == False or (self.table_multi_labels and self.content == 'general'):
                ...
                for word, pos in zip(self.table['words'], self.table['word_positions']):
                    ...
                    if not self.baseline_as_label:
                        ...
                        label_height = self.font.font.getsize(char)[0][1]
                        offset_y = ascent - label_height
                    label[x:x+w, y+offset_y:y+ascent] = self.text_label
            ...
            if self.table_multi_labels:
                ...
                elif self.content == 'line_separator':
                    for l in self.table['lines']:
                               
                        label[self.pos_x : self.pos_x + self.content_width, self.pos_y + l - 3 : self.pos_y + l + 3] = self.line_label
            
                elif self.content == 'column_separator':
                    for c in self.table['columns']:
                        
                        label[self.pos_x + c - 3 : self.pos_x + c + 3, self.pos_y : self.pos_y + self.content_height] = self.column_label
...
#Class corresponding to medieval handwritten tables (especially, astronomical tables), with compact layouts
class TableElement_compact(AbstractElement):
    label = TABLE_LABEL
    color = TABLE_COLOR
    border_label = TEXT_BORDER_LABEL
    border_color = TEXT_BORDER_COLOR
    ...
    def generate_content(self):
        ...
        weights = np.array([TABLE_FREQ[e] for e in TABLE_FREQ.keys()], dtype=np.float64)        
        weights /= weights.sum()
        table_type = choice([*TABLE_FREQ], p=weights)
        ...
        self.table, self.content_width, self.content_height = self._generate_table(dictionary, text)
        self.pos_x = randint(0, max(0, self.width - self.content_width))
        self.pos_y = randint(0, max(0, self.height - self.content_height))

    def _generate_table(self, dictionary, text):

        if(self.width > 1500):
            width, height = randint(min(1500, self.width), self.width), randint(min(1000, self.height), self.height)
        else:
            width, height = randint(min(600, self.width), self.width), randint(min(1000, self.height), self.height)
            
        line_size_min = round(self.font_size * 1.5)
        line_size_max = round(self.font_size * 2)
        lines = np.cumsum(np.random.randint(line_size_min, line_size_max, 90))
        lines = lines[lines < height - line_size_min].tolist()
        columns_size = np.random.randint(25, 35, 60)
        columns_size = np.random.randint(line_size_min, line_size_max, 60)
        
        columns = np.cumsum(columns_size)
        columns = columns[columns < width - TABLE_LAYOUT_RANGE['col_size_range_new'][0]].tolist()
            ...
            if(presence_column):
                for j, l in enumerate([0] + lines):
                    ...
                    if(presence_word):
                        ...
                        cell_width = columns[i] - c if i < len(columns) else width - c
                        cell_height = lines[j] - l if j < len(lines) else height - l
                        while self.font.getsize(word)[0] - 10 + 2 * self.padding > cell_width and len(word) > 0:
                            word = word[:-1].strip()
                        if len(word) > 0:
                            w, h = self.font.getsize(word)
                            p_c, p_l = (cell_width - w) // 2, (cell_height - h) // 2
    ...
    def to_image(self):
        ...
        for l in self.table['lines']:
            ...
            self.line_params = {
                'width': 0,
                'fill': tuple([randint(0, 1)] * 3) + (self.opacity,),
            }
        ...
        for word, pos in zip(self.table['words'], self.table['word_positions']):
            ...
            opacity = randint(*NEG_ELEMENT_OPACITY_RANGE[self.name] if self.as_negative
                               else POS_ELEMENT_OPACITY_RANGE['table_word'])
            
            self.colors_alpha = self.colors + (opacity,)
            pos = pos[0] + self.pos_x, pos[1] + self.pos_y
            draw.text(pos, word, font=self.font, fill=self.colors_alpha)
    ...
    def to_label_as_img(self):
        arr = self.to_label_as_array()
        res = np.zeros(arr.shape + (3,), dtype=np.uint8)
        res[arr == self.line_label] = self.line_color
        res[arr == self.label] = self.color
        res[arr == self.text_label] = self.text_color
        res[arr == self.column_label] = self.column_color
       
        if self.with_border_label:
            res[arr == self.border_label] = self.border_color
        return Image.fromarray(res)
        
#Class corresponding to astronomical diagrams, with random combinations of lines, circles and rectangles
class DiagramElement(AbstractElement):
    ...
    def generate_content(self):
        ...
        self.number_circles = np.random.randint(1,5)
        self.number_rectangles = np.random.randint(0,4)
        self.number_lines = np.random.randint(0,4)
        self.number_words = np.random.randint(0,5)
        ...
        self.table, self.content_width, self.content_height = self._generate_diagram(dictionary, text)
        self.pos_x = randint(0, max(0, self.width - self.content_width))
        self.pos_y = randint(0, max(0, self.height - self.content_height))

    def _generate_diagram(self, dictionary, text):
        width, height = randint(min(300, self.width), self.width), randint(min(300, self.height), self.height)
  
        circle_pos, circle_radius = [], []
        for i in range(self.number_circles):
            radius = randint(min(20, min(width, height)//2.1), min(width, height)//2.1)
        ...
        rectangle_pos, rectangle_sizes = [], []
        for i in range(self.number_rectangles):
            w = randint(min(20, width), width-1)
            h = randint(min(20, height), height-1)
            rectangle_sizes.append((w,h))
            top_left_x = np.random.randint(0, width - w)
            top_left_y = np.random.randint(0, height - h)
            rectangle_pos.append((top_left_x, top_left_y))
             
        line_coords = []
        for i in range(self.number_lines):
            length = randint(0, min(width//2 - 1, height//2 - 1))
            coords_x = randint(length, width - length)
            coords_y = randint(length, height - length)
            
            direction = choice([1, -1])
            line_coords.append((coords_x - length, coords_y - direction * length, coords_x + length, coords_y + direction * length))
        ...
        return ({'circle_pos': circle_pos, 'circle_sizes': circle_radius, 'rectangle_pos': rectangle_pos, 'rectangle_sizes': rectangle_sizes, 'line_coords': line_coords, 'words': words, 'word_positions': word_positions}, width, height)
    ...
    def to_image(self):
        ...
        for circle_pos, circle_size in zip(self.table['circle_pos'], self.table['circle_sizes']):
            ...
            params = {
                'fill': None,
                'outline': (0,0,0,self.opacity)
                
            }
            
            center = [self.pos_x + circle_pos[0], self.pos_y + circle_pos[1]]
            draw.ellipse([center[0] - circle_size, center[1] - circle_size, center[0] + circle_size, center[1] + circle_size], **params)
                
        for rectangle_pos, rectangle_sizes in zip(self.table['rectangle_pos'], self.table['rectangle_sizes']):
            ...
            top_left = [self.pos_x + rectangle_pos[0], self.pos_y + rectangle_pos[1]]
            draw.rectangle([top_left[0], top_left[1], top_left[0] + rectangle_sizes[0], top_left[1] + rectangle_sizes[1]], **params)  
        ...
        for line_coords in self.table['line_coords']:
            ...
            draw.line([self.pos_x + line_coords[0], self.pos_y + line_coords[1], self.pos_x + line_coords[2], self.pos_y + line_coords[3]], **params)      
    ...
    def to_label_as_array(self):
        ...
        def create_circular_mask(w, h, center, radius):
        ...
        if self.as_negative:
            ...
            for circle_radius, pos in zip(self.table['circle_sizes'], self.table['circle_pos']):
                mask = create_circular_mask(w, h, center=(self.pos_x + pos[0], self.pos_y + pos[1]), radius=circle_radius)
                label[mask] = self.label
                
            for rectangle_sizes, pos in zip(self.table['rectangle_sizes'], self.table['rectangle_pos']):
                top_left = [self.pos_x + pos[0], self.pos_y + pos[1]]
                label[top_left[0]:top_left[0] + rectangle_sizes[0],top_left[1]:top_left[1] + rectangle_sizes[1]] = self.label
            
            for line_coords in self.table['line_coords']:                
                if line_coords[1] < line_coords[3]:
                    for i in range(1, line_coords[2] - line_coords[0] - 1):
                        label[self.pos_x + line_coords[0] + i, self.pos_y + line_coords[1] + i] = self.label
