...
from utils.constant import (SEG_GROUND_TRUTH_FMT, LABEL_TO_COLOR_MAPPING, CONTEXT_BACKGROUND_LABEL,
                            ILLUSTRATION_COLOR, ILLUSTRATION_LABELS, TEXT_COLOR, TEXT_LABELS, BACKGROUND_COLOR, TEXT_BORDER_LABEL, TABLE_LABELS)
...
from synthetic.element import (BackgroundElement, ContextBackgroundElement, DrawingElement, GlyphElement, ImageElement,
                               TitleElement, ParagraphElement, CaptionElement, WordElement, TableElement_loose, TableElement_compact, DiagramElement,
...
#Key layout parameters to modify to play with the synthetic document generation:
LAYOUT_RANGE = {
    'nb_h': (1, 1),
    'nb_v': (1, 2),
    'nb_v_lines': (0, 2),
...
#Element frequences to modify to easely adapt the synthetic dataset to our goals. In our case (the study of astronomical manuscripts), the elements of interest were: TableElement_loose, TableElement_compact, DiagramElement (which are defined in element.py)
ELEMENT_FREQ = {
    DrawingElement: 0.05,
    GlyphElement: 0.05,
    ImageElement: 0.05,
    ParagraphElement: 0.2,
    TableElement_loose: 0.05,
    TableElement_compact: 0.4,
    TitleElement: 0.05,
    DiagramElement: 0.15,
}
...
class SyntheticDocument:
    available_elements = [DrawingElement, GlyphElement, ImageElement, ParagraphElement, TableElement_loose, TableElement_compact, TitleElement, DiagramElement]

    @use_seed()
    def __init__(self, width=DEFAULT_DOCUMENT_WIDTH, height=DEFAULT_DOCUMENT_HEIGHT, img_size=None,
                 text_border_label=True, baseline_as_label=False, merged_labels=True, table_multi_labels=False, content = None):
        if height is None:
    ...
    def _generate_random_layout(self, as_negative=False):
        ...
        element_kwargs = {'as_negative': as_negative, 'font_path': font_path, 'with_border_label': not as_negative and
                          self.text_border_label, 'baseline_as_label': self.baseline_as_label, 'table_multi_labels': self.table_multi_labels, 'content': self.content}
        ...
        if(len(self.backgrounds) == 2) and choice([True, False], p=[0, 1]):
            bg_width, bg_height = [self.backgrounds[0]['element'].size[0] + self.backgrounds[1]['element'].size[0], self.backgrounds[0]['element'].size[1]]
            bg_x, bg_y = self.backgrounds[0]['position']
            bg_width -= self.backgrounds[0]['element'].inherent_left_margin
            bg_x += self.backgrounds[0]['element'].inherent_left_margin
    ...
    def to_label_as_array(self, restricted_labels=None):
        ...
        label = np.zeros(self.size[::-1], dtype=np.float64)
            ...
            if self.table_multi_labels:
                if self.merged_labels:
                    if label in ILLUSTRATION_LABELS:
                        if self.content == 'general':
                            res[arr == label] = ILLUSTRATION_COLOR
                        else:
                            res[arr == label] = BACKGROUND_COLOR
                    elif label in TEXT_LABELS:
                        if self.content == 'general':
                            res[arr == label] = TEXT_COLOR
                        else:
                            res[arr == label] = BACKGROUND_COLOR
                    elif label == TEXT_BORDER_LABEL and self.content != 'general':
                        res[arr == label] = BACKGROUND_COLOR
                    else:
                        res[arr == label] = color                   
                else:
    ...
    def _save_label_dist(self, name, output_dir):
        arr = asarray(self.to_label_as_img())
        savez_compressed(output_dir / name, arr)
