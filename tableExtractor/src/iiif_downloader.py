...
class IIIFDownloader:
    ...
    def run(self):
        for url in self.manifest_urls:
            ...
            if manifest is not None:
                ...
                resources = self.get_resources(manifest, url)
    ...
    @staticmethod
    def get_resources(manifest, url):
        try:
            canvases = manifest['sequences'][0]['canvases']
            images_list = [canvas['images'] for canvas in canvases]
            if "vatlib" not in url:
                return [image['resource']['@id'] for images in images_list for image in images]
            else:    
                return [image['resource']['service']['@id'] + '/full/full/0/native.jpg' for images in images_list for image in images]
