...
class Extractor:
    ...
    def run(self):
        ...
        for filename in self.files:
            ...
            for img, name in imgs_with_names:
                ...
                pred_1, pred_2, pred_3, pred_4, _ = self.predict(img)

                for label in self.labels_to_extract:
                    if label != BACKGROUND_LABEL:
                                                
                        if label in self.restricted_labels_1:
                            extracted_elements = self.extract(img, pred_1, label, img_with_annotations)  
                                                      
                        elif label in self.restricted_labels_2:
                                extracted_elements = self.extract(img, pred_2, label, img_with_annotations)
                                
                        elif label in self.restricted_labels_3:
                            extracted_elements = self.extract(img, pred_3, label, img_with_annotations) 
                            
                        elif label in self.restricted_labels_4:
                            extracted_elements = self.extract(img, pred_4, label, img_with_annotations)
    ...
    def predict(self, image):
        ...
        #Compute of softmax to compute mean Average Precision (mAP) metric
        probs = torch.nn.Softmax(dim=0)
        ...
        with torch.no_grad():
            ...
            #Compute of softmax to compute mean Average Precision (mAP) metric
            softmax_output_1 = probs(outputs[1][0])
                        
        return pred_1, pred_2, pred_3, pred_4, softmax_output_1

    def extract(self, image, pred, label, image_with_annotations, myfile = ''):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-verb', '--verbose', action='store_true', help='Whether to be in verbose mode')
    ...
    extractor = Extractor(input_dir, args.output_dir, labels_to_extract=args.labels, tag=args.tag,
                          ...
                          draw_margin=args.draw_margin, verbose=args.verbose)
    extractor.run()
